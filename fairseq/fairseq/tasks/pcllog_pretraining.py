...
class PCLLogConfig(FairseqDataclass):
    ...
    entry_level: bool = field(
        default=False,
        metadata={"help": "entry-level inputs"}
    )
    sequence_level: bool = field(
        default=False,
        metadata={"help": "sequence-level inputs"}
    )
    sequence_insert_delete: float = field(
        default=0.2,
        metadata={"help": "randomly shuffle sentences for this proportion of inputs"},
    )
    sequence_disorder: float = field(
        default=0.5,
        metadata={"help": "randomly shuffle sentences for this proportion of inputs"},
    )
...
class PCLLogTask(FairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = PCLLogDataset(
            ...
            entry_level=self.cfg.entry_level,
            sequence_level=self.cfg.sequence_level,
            negative_sample=self.cfg.negative_sampling,
            sequence_insert_delete=self.cfg.sequence_insert_delete,
